...
def gompertz(x, a, b, c):
    # 3-parameter Gompertz: y = a * exp(-b * exp(-c*x))
...
def generate_combined_plot(fit_results_dict, threshold,
    ...
    for row in summary_rows:
        ...
        label  = (f"{sample} "
                  f"(TT={row['Threshold Time']:.2f}±{row['TT SE']:.2f}, "
...
def create_excel_report(data, fit_results, fit_params,
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        if param_rows:
            pd.DataFrame(param_rows).to_excel(writer,
                sheet_name="Fit Parameters", index=False)
...
if st.checkbox("Use manual calibration"):
...
if not data.empty and len(data.columns) > 1:
        ...
        with st.expander(col):
            ...
            try:
                if model == "Linear":
                    ...
                    ci_low = y_fit; ci_high = y_fit; pcov = None
                else:
                ...
                img = generate_sample_plot(col, fit_df,
                                           x_label, y_label,
                                           manual_thresh,
                                           tt_val, tt_se, logcfu)
    ...
    combined = generate_combined_plot(fit_results,
                                      manual_thresh,
                                      x_label, y_label,
                                      st.session_state['summary_rows'])
    ...
    st.download_button(
        ...
        data=export_all_plots_zip(
            ...
            x_label, y_label, manual_thresh, combined
        ),
